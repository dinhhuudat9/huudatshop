...
import { useAddBalance } from "@workspace/api-client-react";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (amount < 50000) {
      toast({
        ...
        description: "Số tiền nạp tối thiểu là 50.000đ",
      });
      return;
    }

    addBalanceMutation.mutate({ data: { amount, method } }, {
      onSuccess: () => {
        setIsSuccess(true);
        toast({
          title: "Nạp tiền thành công",
          description: `Đã nạp ${formatVND(amount)} vào tài khoản.`,
        });
      },
      onError: (err) => {
        toast({
          variant: "destructive",
          title: "Lỗi",
          description: err.message || "Đã xảy ra lỗi khi nạp tiền.",
        });
      }
    });
  };
          ...
          {isSuccess ? (
            <Card className="bg-card/40 border-border/50 border-green-500/30 text-center py-12">
              <CardContent className="flex flex-col items-center justify-center space-y-4">
                ...
                <h2 className="text-2xl font-bold text-green-500">Nạp tiền thành công!</h2>
                <p className="text-muted-foreground max-w-md">
                  Số tiền <span className="font-bold text-foreground">{formatVND(amount)}</span> đã được cộng vào tài khoản của bạn.
                </p>
                <div className="mt-8 flex gap-4">
                  <Button variant="outline" onClick={() => setIsSuccess(false)}>Nạp thêm</Button>
                  <Button asChild className="box-glow">
                    <Link href="/san-pham">Mua mã nguồn ngay</Link>
                  </Button>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-card/40 border-border/50">
              <CardHeader>
                ...
                <CardDescription>
                  Chọn số tiền và phương thức thanh toán để nạp tiền vào tài khoản
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div className="space-y-4">
                    ...
                    <RadioGroup value={method} onValueChange={(v: any) => setMethod(v)} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <Label
                          htmlFor="method-momo"
                          className="flex flex-col items-center justify-between rounded-lg border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-[#A50064] [&:has([data-state=checked])]:border-[#A50064] cursor-pointer"
                        >
                  ...
                  <div className="bg-muted/30 rounded-lg p-6 border border-border/50 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div>
                      <p className="text-sm text-muted-foreground">Tổng tiền thanh toán</p>
                      <p className="text-3xl font-bold font-mono text-primary text-glow">{formatVND(amount || 0)}</p>
                    </div>
                    <Button type="submit" size="lg" className="w-full sm:w-auto h-14 px-8 text-lg font-bold box-glow" disabled={addBalanceMutation.isPending}>
                      {addBalanceMutation.isPending ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <ArrowRight className="mr-2 h-5 w-5" />}
                      Thanh toán ngay
                    </Button>
                  </div>
                </form>
